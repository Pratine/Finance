...
export default function InvestmentsPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      {refreshMsg && (
        <div className={`mb-4 text-sm bg-slate-50 dark:bg-slate-900 border rounded-xl px-4 py-3 ${refreshErrors.length ? 'border-amber-300 dark:border-amber-700' : 'border-slate-200 dark:border-slate-700'}`}>
          <div className="flex justify-between gap-4">
            <span className={refreshErrors.length ? 'text-amber-700 dark:text-amber-400' : 'text-slate-600 dark:text-slate-400'}>{refreshMsg}</span>
            <button onClick={() => { setRefreshMsg(null); setRefreshErrors([]) }} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-400 shrink-0">✕</button>
          </div>
          {refreshErrors.length > 0 && (
            <ul className="mt-2 space-y-0.5">
              {refreshErrors.map((e, i) => <li key={i} className="text-xs text-amber-600 dark:text-amber-500">· {e}</li>)}
            </ul>
          )}
        </div>
      )}
