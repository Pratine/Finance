...
// ─── Category-specific add row (includes type selector) ───────────────────────
...
const REFRESH_OPTIONS: { value: RefreshInterval; label: string; description: string }[] = [
  ...
  { value: '4h',      label: 'Every 4 hours',   description: 'Recommended — balances freshness and API usage' },
  { value: '8h',      label: 'Every 8 hours',   description: 'Morning / midday / evening refresh' },
  { value: '24h',     label: 'Every 24 hours',  description: 'Once a day — lightest API usage' },
]
...
export default function SettingsPage() {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setError(e?.message ?? 'Cannot delete — item may still be in use')
      setDeleteConfirm(null)
    }
  }
